...
export class AdvancedAnalyzer {
  static async performStepByStepAnalysis(data: any[], config: any) {
    ...
    const missingFactors = variablesToCheck.filter((v: string) => !availableColumns.includes(v));
  ...
  private static async performANOVA(data: any[], targetVariable: string, factorVariables: string[], options: any) {
    ...
    for (const [groupKey, groupData] of Object.entries(groups)) {
      const values = (groupData as any[]).map((row: any) => parseFloat(row[targetVariable])).filter((val: number) => !isNaN(val));
      if (values.length > 0) {
    ...
    (result as any).groupStats = groupStats;
      ...
      return new Promise((resolve, reject) => {
        ...
        pythonProcess.on('close', (code) => {
          try {
            ...
            reject(new Error(`Failed to parse Python analysis results: ${(error as Error).message || String(error)}`));
          }
        });
      });
  ...
  private static async performComprehensiveAnalysis(data: any[], targetVariable: string, variables: string[], config: any) {
    try {
      ...
      return {
        ...
        variables,
        results: {
          error: `Business insights analysis failed: ${(error as Error).message || String(error)}`
        }
      };
    }
  }
  ...
  private static async performComparativeAnalysis(data: any[], targetVariable: string, variables: string[], config: any) {
    try {
      ...
      return {
        ...
        variables,
        results: {
          error: `Comparative analysis failed: ${(error as Error).message || String(error)}`
        }
      };
    }
  }
  ...
  private static async performPredictiveInsights(data: any[], targetVariable: string, variables: string[], config: any) {
    try {
      ...
      return {
        ...
        variables,
        results: {
          error: `Predictive insights analysis failed: ${(error as Error).message || String(error)}`
        }
      };
    }
  }
  ...
  private static async performRootCauseAnalysis(data: any[], targetVariable: string, variables: string[], config: any) {
    try {
      ...
      return {
        ...
        variables,
        results: {
          error: `Root cause analysis failed: ${(error as Error).message || String(error)}`
        }
      };
    }
  }
  ...
  private static groupDataByFactors(data: any[], factorVariables: string[]) {
    ...
    data.forEach(row => {
      const groupKey = factorVariables.map((factor: string) => row[factor]).join('_');
      if (!groups[groupKey]) {
        groups[groupKey] = [];
      }

...
import { users, projects, enterpriseInquiries, guidedAnalysisOrders, datasets, projectDatasets, projectArtifacts, streamingSources, streamChunks, streamCheckpoints, scrapingJobs, scrapingRuns, datasetVersions, agentCheckpoints, projectSessions, projectStates, semanticLinks } from "@shared/schema";
...
// InsertProjectArtifact: id, createdAt, updatedAt can be provided or generated by storage
// Using Partial for optional id and Omit for always-generated timestamps
type InsertProjectArtifact = Partial<Pick<typeof projectArtifacts.$inferInsert, 'id'>> &
  Omit<typeof projectArtifacts.$inferInsert, 'id' | 'createdAt' | 'updatedAt'>;
...
type SemanticLink = typeof semanticLinks.$inferSelect;
type InsertSemanticLink = typeof semanticLinks.$inferInsert;
...
function projectToDataProject(project: Project, dataset?: Dataset, projectState?: Record<string, unknown> | ProjectStateRow): DataProject {
  ...
  const result: DataProject & { preview?: unknown; sampleData?: unknown; journeyProgress?: unknown } = {
    ...
    processed,
    // Database lifecycle fields - FIX: Include all projects table columns for proper typing
    status: projectRecord?.status ?? 'draft',
    createdAt: projectRecord?.createdAt ?? undefined,
    updatedAt: projectRecord?.updatedAt ?? undefined,
    lastModified: projectRecord?.updatedAt ?? projectRecord?.createdAt ?? undefined,
    // Journey lifecycle fields
    lastArtifactId: projectRecord?.lastArtifactId ?? undefined,
    consultationProposalId: projectRecord?.consultationProposalId ?? undefined,
    approvedPlanId: projectRecord?.approvedPlanId ?? undefined,
    analysisExecutedAt: projectRecord?.analysisExecutedAt ?? undefined,
    analysisBilledAt: projectRecord?.analysisBilledAt ?? undefined,
    lockedCostEstimate: projectRecord?.lockedCostEstimate ?? undefined,
    // Journey state tracking fields
    stepCompletionStatus: projectRecord?.stepCompletionStatus ?? undefined,
    lastAccessedStep: projectRecord?.lastAccessedStep ?? undefined,
    journeyStartedAt: projectRecord?.journeyStartedAt ?? undefined,
    journeyCompletedAt: projectRecord?.journeyCompletedAt ?? undefined,
    piiAnalysis,
...
export interface IStorage {
  ...
  // P0 FIX: Atomic journeyProgress merge to prevent race conditions
  // Uses PostgreSQL jsonb_concat at database level - concurrent updates won't overwrite each other
  atomicMergeJourneyProgress(projectId: string, progressUpdate: Record<string, any>): Promise<Record<string, any> | undefined>;
  ...
  // Semantic links - Question → Element → Transformation → Analysis traceability
  createSemanticLink(link: Omit<InsertSemanticLink, 'id' | 'createdAt'>): Promise<SemanticLink>;
  getSemanticLinks(projectId: string, linkType?: string, sourceId?: string): Promise<SemanticLink[]>;
  getSemanticLinkChain(projectId: string, startId: string): Promise<SemanticLink[]>;
  deleteProjectSemanticLinks(projectId: string): Promise<void>;
}
...
export class MemStorage implements IStorage {
  ...
  // P0 FIX: Atomic journeyProgress merge for MemStorage (in-memory deep merge)
  async atomicMergeJourneyProgress(projectId: string, progressUpdate: Record<string, any>): Promise<Record<string, any> | undefined> {
    const existing = this.projects.get(projectId);
    if (!existing) {
      return undefined;
    }

    const currentProgress = (existing as any).journeyProgress || {};
    ...
    const mergedProgress = deepMerge(currentProgress, {
      ...progressUpdate,
      updatedAt: new Date().toISOString()
    });

    const updated = { ...existing, journeyProgress: mergedProgress } as DataProject;
    this.projects.set(projectId, updated);

    console.log(`📋 [MemStorage] atomicMergeJourneyProgress for ${projectId}:`, {
      keysUpdated: Object.keys(progressUpdate),
      hasRequirementsDoc: !!mergedProgress.requirementsDocument
    });
  ...
  async createArtifact(artifactData: InsertProjectArtifact): Promise<ProjectArtifact> {
    ...
    const id = artifactData.id || `artifact_${this.nextId++}`;
  ...
  // Semantic links for MemStorage
  private semanticLinksMap: Map<string, SemanticLink> = new Map();

  async createSemanticLink(link: Omit<InsertSemanticLink, 'id' | 'createdAt'>): Promise<SemanticLink> {
    const id = `slink_${nanoid(10)}`;
    ...
    this.semanticLinksMap.set(id, semanticLink);
    return semanticLink;
  }

  async getSemanticLinks(projectId: string, linkType?: string, sourceId?: string): Promise<SemanticLink[]> {
    return Array.from(this.semanticLinksMap.values())
      .filter(link => {
        if (link.projectId !== projectId) return false;
        if (linkType && link.linkType !== linkType) return false;
        if (sourceId && link.sourceId !== sourceId) return false;
        return true;
      })
      .sort((a, b) => {
  ...
  async getSemanticLinkChain(projectId: string, startId: string): Promise<SemanticLink[]> {
    const chain: SemanticLink[] = [];
    const visited = new Set<string>();
    let currentId = startId;

    while (currentId && !visited.has(currentId)) {
      visited.add(currentId);
      const links = await this.getSemanticLinks(projectId, undefined, currentId);
      if (links.length === 0) break;
      chain.push(...links);
      // Follow to next target
      currentId = links[0]?.targetId || '';
    }
    return chain;
  }

  async deleteProjectSemanticLinks(projectId: string): Promise<void> {
    for (const [id, link] of this.semanticLinksMap.entries()) {
      if (link.projectId === projectId) {
        this.semanticLinksMap.delete(id);
      }
    }
  }
}
...
export class DatabaseStorage implements IStorage {
  ...
  // P0 FIX: Atomic journeyProgress merge using PostgreSQL row-level locking
  // This prevents race conditions when multiple requests update journeyProgress concurrently
  async atomicMergeJourneyProgress(projectId: string, progressUpdate: Record<string, any>): Promise<Record<string, any> | undefined> {
    // Use the pool imported at module level for reliability
    if (!pool) {
      console.error('❌ [atomicMergeJourneyProgress] Database pool not available - db may not be initialized');
      return undefined;
    }
    ...
    try {
      ...
      // Note: Column is snake_case in DB (journey_progress), but Drizzle returns it as camelCase
      const selectResult = await client.query(
        'SELECT journey_progress FROM projects WHERE id = $1 FOR UPDATE',
        [projectId]
      );
      ...
      const currentProgress = selectResult.rows[0].journey_progress || {};
      ...
      // Update with merged progress
      await client.query(
        'UPDATE projects SET journey_progress = $1 WHERE id = $2',
        [JSON.stringify(mergedProgress), projectId]
      );

      // Commit transaction
      await client.query('COMMIT');

      console.log(`✅ [DatabaseStorage] atomicMergeJourneyProgress for ${projectId}:`, {
        keysUpdated: Object.keys(progressUpdate),
        hasRequirementsDoc: !!mergedProgress.requirementsDocument,
        elementsCount: mergedProgress.requirementsDocument?.requiredDataElements?.length || 0
      });
      ...
      await client.query('ROLLBACK');
      console.error('❌ [atomicMergeJourneyProgress] Transaction failed:', error);
      throw error;
    } finally {
      client.release();
    }
  ...
  // Semantic links for DatabaseStorage
  async createSemanticLink(link: Omit<InsertSemanticLink, 'id' | 'createdAt'>): Promise<SemanticLink> {
  ...
  async getSemanticLinks(projectId: string, linkType?: string, sourceId?: string): Promise<SemanticLink[]> {
    ...
    const conditions = [eq(semanticLinks.projectId, projectId)];
    if (linkType) {
      conditions.push(eq(semanticLinks.linkType, linkType));
    }
    if (sourceId) {
      conditions.push(eq(semanticLinks.sourceId, sourceId));
    }

...
export class MLService {
  ...
  async runAnalysis(request: MLAnalysisRequest, dataPath: string): Promise<MLAnalysisResult> {
    ...
    try {
      ...
      throw new Error(`ML Analysis failed: ${(error as Error).message || String(error)}`);
    }
  }
      ...
      pythonProcess.on('close', (code) => {
        if (code === 0) {
          try {
            ...
            reject(new Error(`Failed to parse ML analysis results: ${(parseError as Error).message || String(parseError)}`));
          }
        } else {

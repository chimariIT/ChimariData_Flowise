...
export class UnifiedPIIProcessor {
  ...
  static async processPIIData(config: PIIProcessingConfig): Promise<PIIProcessingResult> {
    ...
    const effectivePIIColumns = piiAnalysis.detectedPII.filter((col: string) => !overriddenColumns.includes(col));
    ...
    switch (decision) {
      case 'exclude':
        ...
        finalData = originalData.map(row => {
          const cleanRow = { ...row };
          effectivePIIColumns.forEach((piiColumn: string) => {
      ...
      case 'anonymize':
        ...
        if (anonConfig && anonConfig.fieldsToAnonymize) {
          ...
          const fieldsToAnonymize = anonConfig.fieldsToAnonymize.filter((field: string) => !overriddenColumns.includes(field));

    ...
    async queueWorkflowTasks(projectId: string, tasks: Array<{
        ...
        for (const task of sortedTasks) {
            ...
            const taskId = await this.queueTask({
                ...
                dependencies: (dependencies || []).filter((d): d is string => d !== undefined),
    ...
    async coordinateGoalAnalysis(
        ...
        if (!projectId || typeof projectId !== 'string') {
            return {
                ...
                expertOpinions: [],
                synthesis: {
                    overallAssessment: 'not_feasible' as const,
                    confidence: 0,
                    keyFindings: ['Project ID must be a valid string'],
                    combinedRisks: [{ source: 'pm_agent', risk: 'Invalid project identification', severity: 'high' as const }],
                    actionableRecommendations: ['Please provide a valid project ID'],
                    expertConsensus: {
                        dataQuality: 'poor' as const,
                        technicalFeasibility: 'not_feasible' as const,
        ...
        if (!uploadedData || typeof uploadedData !== 'object') {
            return {
                ...
                synthesis: {
                    ...
                    combinedRisks: [{ source: 'pm_agent', risk: 'No data available for analysis', severity: 'high' as const }],
        ...
        if (!userGoals || !Array.isArray(userGoals)) {
            return {
                ...
                synthesis: {
                    ...
                    combinedRisks: [{ source: 'pm_agent', risk: 'Unclear project objectives', severity: 'high' as const }],
        ...
        if (!industry || typeof industry !== 'string') {
            return {
                ...
                synthesis: {
                    ...
                    combinedRisks: [{ source: 'pm_agent', risk: 'Lack of industry context', severity: 'high' as const }],
    ...
    synthesizeExpertOpinions(
        ...
        if (!expertOpinions || !Array.isArray(expertOpinions)) {
            return {
                ...
                combinedRisks: [{ source: 'pm_agent', risk: 'Missing expert input', severity: 'high' as const }],

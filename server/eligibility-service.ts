...
// Type alias for User from storage schema
...
export class EligibilityService {
  ...
  private cleanExpiredCache(): void {
    const now = Date.now();
    const entries = Array.from(this.eligibilityCache.entries());
  ...
  /**
   * Map tier features to the format expected by limits object
   */
  private mapTierLimitsToUsageFormat(tierLimits: any) {
    return {
      monthlyUploads: tierLimits.maxFiles === -1 ? Infinity : tierLimits.maxFiles,
      monthlyDataVolume: tierLimits.totalDataVolumeMB === -1 ? Infinity : tierLimits.totalDataVolumeMB,
      monthlyAIInsights: tierLimits.aiInsights === -1 ? Infinity : tierLimits.aiInsights,
    };
  }
  ...
  public async checkEligibility(
    ...
    try {
      ...
      const limits = this.mapTierLimitsToUsageFormat(tierLimits);
      ...
      const checkRecord = await storage.createEligibilityCheck(checkData as any);
  ...
  public async canUserPerformAction(
    ...
    try {
      ...
      switch (action) {
        case 'upload':
          const uploadCheck = canUserUpload(
            user.subscriptionTier || 'none', 
            user.monthlyUploads || 0, 
            0, // fileSizeMB placeholder
            user.monthlyDataVolume || 0
          );
          return uploadCheck.allowed 
            ? { allowed: true } 
            : { allowed: false, reason: uploadCheck.reason || 'Upload limit exceeded', tier: user.subscriptionTier || 'none' };
            
        case 'ai_insight':
          const insightCheck = canUserRequestAIInsight(
            user.subscriptionTier || 'none',
            user.monthlyAIInsights || 0
          );
          return insightCheck.allowed
            ? { allowed: true }
            : { allowed: false, reason: insightCheck.reason || 'AI insight limit exceeded', tier: user.subscriptionTier || 'none' };
            
        case 'analysis':
          // Check if user has analysis capabilities
          const tierLimits = getTierLimits(user.subscriptionTier || 'none');
          const mappedLimits = this.mapTierLimitsToUsageFormat(tierLimits);
          const hasAnalysisAccess = mappedLimits.monthlyAIInsights > 0;

...
export class PricingService {
  ...
  async calculateIntelligentEstimate(request: {
    ...
    try {
      ...
      if (request.journeyId && (!goals.length || !questions.length)) {
        try {
          // TODO: Implement journey retrieval when journey storage is available
          console.log('Journey retrieval not implemented yet');
  ...
  private async cacheIntelligentEstimate(cacheKey: string, estimate: any): Promise<void> {
    try {
      // Create a simplified estimate for cache storage
      const cacheEstimate = {
        id: nanoid(),
        createdAt: new Date(),
        userId: '',
        estimateType: 'full_journey' as const,
        items: [],
        ...
        signature: '',
        validUntil: new Date(Date.now() + this.ESTIMATE_VALIDITY_MS),
        approved: false,
        journeyId: null,
      ...
      // Store in database for persistence - TODO: implement when needed
      console.log('Intelligent estimate caching not implemented yet');
  ...
  /**
   * Calculate subscription credits based on journey type
   */
  private calculateSubscriptionCredits(journeyType: string): number {
    // Subscription credit amounts in cents
    const subscriptionCredits = {
      'non-tech': 2000, // $20.00 credit for non-tech users
      'business': 5000, // $50.00 credit for business users  
      'technical': 10000, // $100.00 credit for technical users
    };
    
    return subscriptionCredits[journeyType as keyof typeof subscriptionCredits] || 0;
  }
  ...
  private calculatePricing(request: PricingEstimateRequest): PricingCalculation {
    ...
    // Add subscription client credits if applicable
    // This would typically be retrieved from user's subscription tier
    const subscriptionCredits = this.calculateSubscriptionCredits(request.journeyType);
    if (subscriptionCredits > 0) {
      items.push({
        description: `Subscription client credits (${request.journeyType} tier)`,
        quantity: 1,
        unitPrice: -subscriptionCredits, // Negative price for credit
        total: -subscriptionCredits,
      });
      subtotal -= subscriptionCredits;
    }
    ...
    try {
      ...
      if (cached && Date.now() - cached.timestamp < cached.ttlMs) {
        ...
        return {
          ...
          currency: cached.estimate.currency || 'USD',
      ...
      this.estimateCache.set(inputsHash, {
        estimate: estimate as CostEstimate,
        timestamp: Date.now(),
        ttlMs: this.CACHE_TTL_MS,
      });
  ...
  public async verifyEstimate(estimateId: string, signature: string): Promise<{
    ...
    try {
      ...
      return { valid: true, estimate: estimate as CostEstimate };
  ...
  public async confirmEstimate(
    ...
    try {
      ...
      return { success: true, estimate: updatedEstimate as CostEstimate };

...
import { datasets, projectDatasets, projectArtifacts, DataProject } from '@shared/schema';

// Use local type aliases derived from Drizzle tables since schema doesn't export these
...
export class MigrationService {
  ...
  private async createDatasetFromProject(project: LegacyProjectData): Promise<Dataset> {
    ...
    const dataset = await storage.createDataset({
      ...
      storageUri: `legacy://${project.id}/${project.fileName}`,
      recordCount: project.recordCount || (project.data?.length || 0),
      schema: project.schema || {},
      preview: project.data?.slice(0, 5) || [],
      piiAnalysis: {
        ...(project.piiAnalysis || {}),
        detectedPII: project.piiAnalysis?.detectedPII || []
      },
      ingestionMetadata: {
  ...
  private async createArtifactsFromProject(project: LegacyProjectData): Promise<ProjectArtifact[]> {
    ...
    if (project.transformations) {
      const artifact = await storage.createArtifact({
        ...
        type: 'transformation',
        status: 'completed',
        params: { description: 'Migrated data transformations from legacy project' },
        output: project.transformations
      });
    ...
    if (project.analysisResults) {
      const artifact = await storage.createArtifact({
        ...
        type: 'analysis',
        status: 'completed',
        params: { description: 'Migrated analysis results from legacy project' },
        output: project.analysisResults,
    ...
    if (project.stepByStepAnalysis) {
      const artifact = await storage.createArtifact({
        ...
        type: 'analysis',
        status: 'completed',
        params: { description: 'Migrated guided analysis from legacy project', type: 'guided_analysis' },
        output: project.stepByStepAnalysis,
        parentArtifactId: lastArtifactId
      });
      artifacts.push(artifact);
      lastArtifactId = artifact.id;
    ...
    if (project.visualizations) {
      const artifact = await storage.createArtifact({
        ...
        type: 'visualization',
        status: 'completed',
        params: { description: 'Migrated visualizations from legacy project' },
        output: project.visualizations,
        parentArtifactId: lastArtifactId
      });
      artifacts.push(artifact);
      lastArtifactId = artifact.id;
    ...
    if (project.aiInsights) {
      const artifact = await storage.createArtifact({
        ...
        type: 'report',
        status: 'completed',
        params: { description: 'Migrated AI insights from legacy project', type: 'ai_insight' },
        output: project.aiInsights,
        parentArtifactId: lastArtifactId
      });
      artifacts.push(artifact);
      lastArtifactId = artifact.id;
    ...
    if (project.outlierAnalysis) {
      const artifact = await storage.createArtifact({
        ...
        type: 'analysis',
        status: 'completed',
        params: { description: 'Migrated outlier analysis from legacy project', type: 'outlier_analysis' },
        output: project.outlierAnalysis,
        parentArtifactId: lastArtifactId
      });
      artifacts.push(artifact);
      lastArtifactId = artifact.id;
    ...
    if (project.missingDataAnalysis) {
      const artifact = await storage.createArtifact({
        ...
        type: 'analysis',
        status: 'completed',
        params: { description: 'Migrated missing data analysis from legacy project', type: 'missing_data' },
        output: project.missingDataAnalysis,
        parentArtifactId: lastArtifactId
      });
      artifacts.push(artifact);
      lastArtifactId = artifact.id;
    ...
    if (project.normalityTests) {
      const artifact = await storage.createArtifact({
        ...
        type: 'analysis',
        status: 'completed',
        params: { description: 'Migrated normality tests from legacy project', type: 'normality_tests' },
        output: project.normalityTests,
        parentArtifactId: lastArtifactId
      });
      artifacts.push(artifact);
      lastArtifactId = artifact.id;
    ...
    if (project.joinedFiles) {
      const artifact = await storage.createArtifact({
        ...
        type: 'transformation',
        status: 'completed',
        params: { description: 'Migrated joined files from legacy project', type: 'data_join' },
        output: project.joinedFiles,
        parentArtifactId: lastArtifactId
      });
      artifacts.push(artifact);
      lastArtifactId = artifact.id;
    ...
    // Update project with last artifact reference (only if supported by schema)
    // if (lastArtifactId) {
  ...
  private async safeCleanupLegacyProject(project: LegacyProjectData, datasetId?: string): Promise<void> {
    ...
    await storage.updateProject(project.id, {
      ...
      fileType: undefined,
      schema: undefined,
      data: undefined, // SAFE: Data is now preserved in dataset

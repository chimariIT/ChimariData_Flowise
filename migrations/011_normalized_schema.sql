...
-- Normalized schema with pgvector for semantic search
-- Replaces JSONB black boxes with queryable columns and proper FK constraints
...
-- Note: Vector index created after data population for better performance
-- CREATE INDEX idx_questions_embedding ON project_questions USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
CREATE TABLE IF NOT EXISTS agent_executions (
  ...
  -- References to prior executions this depends on (proper array)
...
CREATE TABLE IF NOT EXISTS ds_analysis_results (
  ...
  -- Only truly variable data in JSONB (like feature lists, confusion matrices)
  additional_metrics JSONB DEFAULT '{}',

  -- Python script that generated this
...
CREATE TABLE IF NOT EXISTS ml_model_artifacts (
  ...
  -- Storage
  artifact_path TEXT, -- path to .pkl/.joblib file
...
-- ============================================
-- VISUALIZATIONS (metadata only, actual files stored separately)
-- ============================================
CREATE TABLE IF NOT EXISTS visualization_artifacts (
  ...
  -- Visualization info
  viz_type VARCHAR(50) NOT NULL, -- heatmap, scatter, bar, line, histogram, etc.
  title VARCHAR(255),
  description TEXT,
  ...
  file_path TEXT,
  thumbnail_path TEXT,
  format VARCHAR(20), -- png, svg, html, json

  -- Config (for interactive regeneration)
  config JSONB,
...
-- Note: Vector index created after data population
-- CREATE INDEX idx_answers_embedding ON question_answers USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
-- View: Project analysis summary
CREATE OR REPLACE VIEW v_project_analysis_summary AS
SELECT
  p.id as project_id,
  ...
  COUNT(DISTINCT dar.id) as analysis_result_count,
  COUNT(DISTINCT i.id) as insight_count,
  COUNT(DISTINCT qa.id) as answered_questions,
  AVG(dar.confidence)::numeric(5,2) as avg_confidence
FROM projects p
LEFT JOIN project_questions pq ON p.id = pq.project_id
LEFT JOIN agent_executions ae ON p.id = ae.project_id
LEFT JOIN ds_analysis_results dar ON ae.id = dar.execution_id
LEFT JOIN insights i ON dar.id = i.analysis_result_id
LEFT JOIN question_answers qa ON pq.id = qa.question_id
GROUP BY p.id, p.name;

-- View: Question with answer status
CREATE OR REPLACE VIEW v_question_status AS
SELECT
  ...
  qa.confidence as answer_confidence,
  COUNT(ec.id) as evidence_chain_length,
  COUNT(ai.insight_id) as supporting_insights
FROM project_questions pq
LEFT JOIN question_answers qa ON pq.id = qa.question_id
LEFT JOIN evidence_chain ec ON qa.id = ec.answer_id
LEFT JOIN answer_insights ai ON qa.id = ai.answer_id
GROUP BY pq.id, pq.project_id, pq.text, pq.complexity, qa.id, qa.confidence;
...
COMMENT ON TABLE project_questions IS 'User questions with embeddings for semantic search';
COMMENT ON TABLE agent_executions IS 'Tracks all agent executions with proper status lifecycle';
COMMENT ON TABLE de_quality_reports IS 'Data Engineer quality metrics - normalized columns, not JSONB';
COMMENT ON TABLE ds_analysis_results IS 'Statistical results with queryable columns for p-values, coefficients, etc.';
COMMENT ON TABLE insights IS 'Analysis insights with embeddings for semantic matching to questions';
COMMENT ON TABLE question_answers IS 'Final answers with full evidence chain';
...
COMMENT ON COLUMN project_questions.embedding IS 'OpenAI ada-002 embedding (1536 dimensions)';
COMMENT ON COLUMN ds_analysis_results.p_value IS 'Statistical significance - queryable for filtering';
COMMENT ON COLUMN ds_analysis_results.additional_metrics IS 'Only truly variable data (confusion matrices, etc.)';

...
Alice,28,Boston`;
    ...
    // Step 5: Test simple file upload with curl-like approach
    console.log('5️⃣ Testing file upload with curl...');
    const { exec } = require('child_process');
    const util = require('util');
    const execPromise = util.promisify(exec);
    
    try {
      const curlCommand = `curl -X POST "${SERVER_URL}/api/upload" -H "Authorization: Bearer ${token}" -F "file=@test_upload.csv" -s`;

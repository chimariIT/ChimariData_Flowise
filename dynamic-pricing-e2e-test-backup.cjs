...
David Lee,41,75000,Sales`;
      ...
      if (analysis.recordCount !== 6 || analysis.columnCount !== 4) {
        throw new Error(`Expected 6 records and 4 columns, got ${analysis.recordCount} records and ${analysis.columnCount} columns`);
      }
...
5,Charlie,Wilson,charlie.wilson@email.com,555-7890,321-65-9870,38,70000`;
  ...
  async testWorkflowManagement() {
    await this.testScenario('Workflow State Management', async () => {
      ...
      if (workflow.currentStep !== 'file_upload') {
        throw new Error('Initial workflow step should be file_upload');
      }

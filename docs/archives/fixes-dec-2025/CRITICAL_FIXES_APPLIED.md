...
## December 1, 2025 – Backend Regression Fixes

| Fix | Area | Files |
| --- | --- | --- |
| Guard plan creation locks + schema-safe defaults | Project Manager Agent | `server/services/project-manager-agent.ts` |
| Replace PricingService with unified billing service | Project Manager Agent & head build | `server/services/project-manager-agent.ts`, `server/services/project-manager-agent.head.ts` |
| Generate business context through BusinessAgent + fallback recommendations | Project Manager Agent | `server/services/project-manager-agent.ts` |
| Normalize lineage parent references to typed arrays | Data lineage service | `server/services/data-lineage.ts` |

### Highlights
- Plan creation now fails fast when Redis lock keys cannot be acquired and releases the held key on errors, preventing duplicate plans.
- Default blueprints, blueprint steps, and business context payloads now align with `analysisPlanSchema`/`businessContextSchema`, eliminating the previous `null`/shape mismatches that broke TypeScript checking.
- Checkout sessions route through `billing/unified-billing-service`, ensuring platform-wide billing metadata stays consistent even in the head build.
- `BusinessAgent.provideBusinessContext` powers project-manager fallbacks, so strategic context shares the same logic/UI expectations as other agents.
- Data lineage traversals sanitize `parentNodes` before iterating, preventing the `{}` runtime shape from tripping recursive impact/provenance lookups.

...
if (-not $SkipJava) {
    ...
    if (Test-Command java) {
        ...
        if (Test-Command java) {
            ...
            Write-Host "[ERROR] Java installation failed or PATH not updated" -ForegroundColor Red
        }
    }
    Write-Host ""
}
...
if (-not $SkipSpark) {
    ...
    if (Test-Path "$sparkDir\bin\spark-submit.cmd") {
        Write-Host "[OK] Apache Spark already installed at $sparkDir" -ForegroundColor Green
        ...
        try {
            ...
            Write-Host "[OK] Spark downloaded successfully" -ForegroundColor Green
            ...
            Write-Host "[OK] Spark extracted successfully" -ForegroundColor Green
            ...
            try {
                ...
                Write-Host "[OK] winutils.exe installed" -ForegroundColor Green
            } catch {
                Write-Host "[Warning] Failed to download winutils.exe: $_" -ForegroundColor Yellow
            ...
            Write-Host "[OK] Apache Spark installed successfully" -ForegroundColor Green
            ...
            Write-Host "[ERROR] Failed to install Spark: $_" -ForegroundColor Red
    ...
    if (Test-Command redis-server) {
        ...
        if (Test-Command redis-server) {
            ...
            try {
                ...
                Write-Host "[Warning] Could not start Redis service automatically" -ForegroundColor Yellow
            ...
            if ($redisPing -eq "PONG") {
                ...
                Write-Host "[Warning] Redis service may not be running" -ForegroundColor Yellow
            }
        } else {
...
if (Test-Path $requirementsPath) {
    try {
        ...
        Write-Host "[Warning] Error installing Python packages: $_" -ForegroundColor Yellow
...
if (Test-Command python) {
    ...
    $services += @{ Name = "Python"; Status = "[OK] Installed"; Version = $pythonVer }
} else {
    $services += @{ Name = "Python"; Status = "[X] Not Found"; Version = "N/A" }
}

if (Test-Command java) {
    $javaVer = java -version 2>&1 | Select-Object -First 1
    $services += @{ Name = "Java"; Status = "[OK] Installed"; Version = $javaVer }
} else {
    $services += @{ Name = "Java"; Status = "[X] Not Found"; Version = "N/A" }
}
...
if (Test-Path "C:\spark\spark\bin\spark-submit.cmd") {
    ...
    $services += @{ Name = "Spark"; Status = "[OK] Installed"; Version = $sparkVer }
} else {
    $services += @{ Name = "Spark"; Status = "[X] Not Found"; Version = "N/A" }
}

if (Test-Command redis-server) {
    $redisVer = redis-server --version 2>&1 | Select-Object -First 1
    $redisPing = redis-cli ping 2>&1
    $redisStatus = if ($redisPing -eq "PONG") { "[OK] Running" } else { "[Warning] Installed but not running" }
    $services += @{ Name = "Redis"; Status = $redisStatus; Version = $redisVer }
} else {
    $services += @{ Name = "Redis"; Status = "[X] Not Found"; Version = "N/A" }
}

foreach ($service in $services) {
    Write-Host ("{0,-15} {1,-40} {2}" -f $service.Name, $service.Status, $service.Version)
}

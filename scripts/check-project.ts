...
import { projects, analysisPlans, decisionAudits, generatedArtifacts } from '../shared/schema.js';
...
async function checkProject() {
  ...
  try {
    ...
    if (project.analysisResults) {
      ...
      console.log(`   Keys: ${Object.keys(results as any).join(', ')}`);
      if ((results as any).insights) {
        console.log(`   Insights count: ${Array.isArray((results as any).insights) ? (results as any).insights.length : 'N/A'}`);
      }
      if ((results as any).recommendations) {
        console.log(`   Recommendations count: ${Array.isArray((results as any).recommendations) ? (results as any).recommendations.length : 'N/A'}`);
      }
    } else {
      ...
      artifacts.forEach((artifact, idx) => {
        ...
        if (artifact.fileRefs) {
          const refs = typeof artifact.fileRefs === 'string'
            ...
            : artifact.fileRefs;
          console.log(`   - Files: ${(refs as any).length} file(s)`);
        }
      });
    } else {
    ...
    if (project.status === 'completed' && !artifacts.length) {
      ...
      console.log('   → This project was completed BEFORE the artifact generation fix');
      console.log('   → You need to run the backfill script to generate artifacts');
    }

    if (plans.length > 0 && audits.length === 0) {
      console.log('2. ⚠️  Analysis plan exists but no decision audits');
      console.log('   → This project was completed BEFORE the decision audit fix');
      console.log('   → You need to run the backfill script to create decision audits');
    }
    ...
    console.log('\n' + '='.repeat(80));
    console.log('💡 SOLUTION: This is an EXISTING project that needs backfill!');
    console.log('='.repeat(80));
    console.log('\nThe fixes we applied only work for NEW projects going forward.');
    console.log('Existing projects need a backfill script to generate:');
    console.log('  - Artifacts (PDF, PPTX, CSV, JSON, Dashboard)');
    console.log('  - Decision audit entries');
    console.log('\nSee SOLUTION_ARTIFACTS_NOT_SHOWING.md for backfill script details.\n');
    console.log('='.repeat(80) + '\n');

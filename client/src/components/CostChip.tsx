  ...
  if (features.length === 0) {
    return (
      <Badge variant="outline" className={`${getSizeClasses()} ${className}`} data-testid="cost-chip-empty">
        {showIcon && <DollarSign className={`${getIconSize()} icon-gap text-gray-400`} />}
        Select features
      </Badge>
    );
  }
  ...
  if (isLoading) {
    return (
      <Badge variant="outline" className={`${getSizeClasses()} ${className}`} data-testid="cost-chip-loading">
        <Loader2 className={`${getIconSize()} icon-gap animate-spin text-blue-500`} />
        Calculating...
      </Badge>
    );
  }
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Badge variant="destructive" className={`${getSizeClasses()} ${className}`} data-testid="cost-chip-error">
              {showIcon && <AlertTriangle className={`${getIconSize()} icon-gap`} />}
              Error
            </Badge>
          </TooltipTrigger>
          <TooltipContent>
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            {showIcon && (
              <div className="flex items-center icon-gap">
                {isExpiringSoon ? (

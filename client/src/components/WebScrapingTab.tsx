...
export function WebScrapingTab({ projectId, onComplete }: WebScrapingTabProps) {
  ...
  // Build login spec
  type LoginSpec = { type: 'form' | 'header' | 'basic'; usernameSelector?: string; passwordSelector?: string; headers?: Record<string, string> } | undefined;
  let loginSpec: LoginSpec = undefined;
      if (data.loginRequired) {
        ...
        if (data.loginType === 'form') {
          ...
          try {
            const parsed = JSON.parse(data.loginHeaders) as Record<string, string>;
            loginSpec = { ...loginSpec, headers: parsed };
          } catch (error) {

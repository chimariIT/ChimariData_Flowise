...
export function DescriptiveStats({ project, analysisResult }: DescriptiveStatsProps) {
  ...
  const numericVariables = availableVariables.filter(variable => 
    (schema && (schema as any)[variable]?.type === 'number') || (schema && (schema as any)[variable]?.type === 'integer')
  );
  const categoricalVariables = availableVariables.filter(variable => 
    (schema && (schema as any)[variable]?.type === 'text') || (schema && (schema as any)[variable]?.type === 'string') || (schema && (schema as any)[variable]?.type === 'boolean')
  );
  ...
  const columnsToProcess = selectedVariables.length > 0 ? selectedVariables : Object.keys(schema || {} as any);
    
    columnsToProcess.forEach(column => {
  if (!schema || !(schema as any)[column]) return;
      const columnData = data.map(row => row[column]).filter(val => val !== null && val !== undefined);
  const columnSchema = (schema as any)[column];
              ...
              {(varStats as any).type === 'numeric' ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {Object.entries((varStats as any).frequencies as Record<string, number>).map(([category, count]: [string, number]) => (
            ...
            <div className="space-y-4">
              ...
              {analysisResult.analysisResult?.groupStats && (
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(analysisResult.analysisResult.groupStats as Record<string, { n: number; mean: number; standardDeviation: number }> ).map(([group, stats]: [string, { n: number; mean: number; standardDeviation: number }]) => (

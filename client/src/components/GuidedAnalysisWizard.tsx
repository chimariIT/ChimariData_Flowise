...
type AnalysisComplexity = 'Basic' | 'Intermediate' | 'Advanced';
type AnalysisTypeKey = 'descriptive' | 'comparative' | 'predictive' | 'diagnostic';
type DeliverableKey = 'executive_summary' | 'detailed_report' | 'data_visualizations' | 'statistical_tables' | 'presentation_deck' | 'action_plan';
type ScenarioId =
  | 'hr_engagement'
  | 'sales_performance'
  | 'marketing_roi'
...
const ANALYSIS_TYPES: Record<AnalysisTypeKey, {
  name: string;
  description: string;
  icon: React.ReactNode;
...
type ScenarioTemplateKey = 'hr_engagement' | 'sales_performance' | 'financial_performance';
const QUESTION_TEMPLATES: Record<ScenarioTemplateKey, Record<AnalysisTypeKey, string[]>> = {
  'hr_engagement': {
    'descriptive': [
...
function isTemplateScenario(id: string): id is ScenarioTemplateKey {
  return (id as ScenarioTemplateKey) in QUESTION_TEMPLATES;
}

const DELIVERABLE_PRICING: Record<DeliverableKey, { name: string; price: number }> = {
...
export default function GuidedAnalysisWizard({ 
  ...
  const [analysisConfig, setAnalysisConfig] = useState({
    ...
    businessQuestion: '',
    dataDescription: '',
    analysisType: '' as '' | AnalysisTypeKey,
    specificQuestions: [] as string[],
    expectedOutcome: '',
    // timeline field removed
    stakeholders: '',
    selectedScenario: null as (BusinessScenario & { id: ScenarioId }) | null,
    customScenario: false,
    selectedVariables: [] as string[],
    hypotheses: [] as string[],
    deliverables: [] as DeliverableKey[],
  ...
  const calculatePricing = () => {
    ...
    const analysisType = ANALYSIS_TYPES[analysisConfig.analysisType as AnalysisTypeKey];
    ...
    const deliverablesPrice = analysisConfig.deliverables.reduce((total, deliverableId) => {
      const deliverable = DELIVERABLE_PRICING[deliverableId as DeliverableKey];
  ...
  const handleScenarioSelect = (scenario: BusinessScenario) => {
    setAnalysisConfig(prev => ({
      ...prev,
      selectedScenario: { ...scenario, id: scenario.id as ScenarioId },
      role: scenario.role,
  ...
  const handlePayment = async () => {
    ...
    try {
      // 1) Ask server for subscription-adjusted billing breakdown to get finalCost (in cents)
      // Derive a minimal payload; if we don't know dataset size, assume 100MB as a conservative default
      const datasetSizeMB = Math.max(1, analysisConfig.selectedVariables?.length ? analysisConfig.selectedVariables.length * 10 : 100);
      const journeyType = 'non-tech';

      let finalCostCents = 0;
      let baseCostCents = 0;
      let creditsCents = 0;
      try {
        const breakdownRes: any = await apiClient.post('/api/billing/journey-breakdown', {
          journeyType,
          datasetSizeMB,
          additionalFeatures: []
        });
        if (breakdownRes?.success && breakdownRes.breakdown) {
          finalCostCents = breakdownRes.breakdown.totalCost || 0;
          baseCostCents = breakdownRes.breakdown.baseCost || 0;
          creditsCents = breakdownRes.breakdown.subscriptionCredits || 0;
        }
      } catch (e) {
        // Soft-fail: if breakdown is unavailable, we’ll fall back to client-side pricing
      }

      // 2) Compute amount in dollars for Stripe from finalCost; fallback to local pricing.total
      const amountDollars = finalCostCents > 0 ? Math.max(0, finalCostCents) / 100 : pricing.total;

      // 3) Create a payment intent (test/mock endpoint)
      const piResp = await fetch('/api/create-payment-intent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: amountDollars, description: 'Guided Analysis', metadata: { journeyType } })
      });
      if (!piResp.ok) {
        const err = await piResp.json().catch(() => ({}));
        throw new Error(err.error || `Failed to create payment intent (${piResp.status})`);
      }
      ...
      if (clientSecret) {
        ...
        // Navigate to checkout page with the final amount
        window.location.href = `/checkout?type=guided_analysis&amount=${amountDollars}`;
      }
    } catch (error: any) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(ANALYSIS_TYPES).map(([key, analysis]) => (
                <Card 
                  key={key}
                  className={`cursor-pointer transition-all hover:shadow-md ${
                    analysisConfig.analysisType === (key as AnalysisTypeKey)
                      ? 'ring-2 ring-blue-500 bg-blue-50' 
                      : ''
                  }`}
                  onClick={() => setAnalysisConfig(prev => ({ ...prev, analysisType: key as AnalysisTypeKey }))}
                >
                  <CardHeader>
              ...
              <Card className="bg-green-50 border-green-200">
                ...
                <CardContent>
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {analysisConfig.selectedScenario.recommendedAnalysis.map((rec: string) => (
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      setAnalysisConfig(prev => ({ 
                        ...prev, 
                        analysisType: analysisType as AnalysisTypeKey,
      ...
      case 5:
        return (
          ...
          {analysisConfig.selectedScenario && analysisConfig.analysisType &&
            isTemplateScenario(analysisConfig.selectedScenario.id) &&
            QUESTION_TEMPLATES[analysisConfig.selectedScenario.id as ScenarioTemplateKey] &&
            QUESTION_TEMPLATES[analysisConfig.selectedScenario.id as ScenarioTemplateKey][analysisConfig.analysisType as AnalysisTypeKey] && (
                <Card className="bg-green-50 border-green-200">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-green-800">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div className="grid grid-cols-1 gap-2">
                        {QUESTION_TEMPLATES[analysisConfig.selectedScenario.id as ScenarioTemplateKey][analysisConfig.analysisType as AnalysisTypeKey].map((question: string, index: number) => (
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const selected = analysisConfig.selectedScenario;
                          const type = analysisConfig.analysisType;
                          if (!selected || !type) return;
                          const sid = selected.id;
                          if (!isTemplateScenario(sid)) return;
                          const templateQuestions = QUESTION_TEMPLATES[sid][type as AnalysisTypeKey];
                          const newQuestions = templateQuestions.filter((q: string) => !analysisConfig.specificQuestions.includes(q));
                          setAnalysisConfig(prev => ({
                            ...prev,
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {analysisConfig.specificQuestions.map((question: string, index: number) => (
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {analysisConfig.hypotheses.map((hypothesis: string, index: number) => (
                        <Badge key={index} variant="secondary" className="text-xs">
                          {hypothesis}
                          <Button
                            variant="ghost"
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    ] as Array<{ id: DeliverableKey; label: string; description: string }>).map((deliverable) => (
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            if (checked) {
                              ...
                              setAnalysisConfig(prev => ({
                                ...prev,
                                deliverables: prev.deliverables.filter((d: DeliverableKey) => d !== deliverable.id)
                              }));
                            }
                          }}
                        />
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{analysisConfig.analysisType ? ANALYSIS_TYPES[analysisConfig.analysisType as AnalysisTypeKey].name : 'Not selected'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {analysisConfig.deliverables.slice(0, 2).map((deliverable: DeliverableKey) => (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {analysisConfig.analysisType ? ANALYSIS_TYPES[analysisConfig.analysisType as AnalysisTypeKey].name : 'Selected Analysis'}
                        </p>
                      </div>
                    </div>

    ...
    const calculateApproachCosts = async () => {
      ...
      for (const approach of approaches) {
        try {
          ...
          const complexityMap: Record<'basic' | 'intermediate' | 'advanced', 'basic' | 'intermediate' | 'advanced'> = {
            basic: 'basic',
            intermediate: 'intermediate',
            advanced: 'advanced',
          };
          const rawComplexity = String(approach?.complexity || '').toLowerCase();
          const allowed = new Set(['basic','intermediate','advanced'] as const);
          const complexityKey: 'basic' | 'intermediate' | 'advanced' = (allowed.has(rawComplexity as any)
            ? (rawComplexity as 'basic' | 'intermediate' | 'advanced')
            : 'intermediate');
          const complexity = complexityMap[complexityKey];
...
function calculateFallbackCost(approach: any, goalCount: number, questionCount: number): number {
  const baseRateMap: Record<'basic' | 'intermediate' | 'advanced', number> = {
    basic: 2500,
    intermediate: 3500,
    advanced: 5000,
  };
  const raw = String(approach?.complexity || '').toLowerCase();
  const allowed = new Set(['basic','intermediate','advanced'] as const);
  const key: 'basic' | 'intermediate' | 'advanced' = allowed.has(raw as any)
    ? (raw as 'basic' | 'intermediate' | 'advanced')
    : 'intermediate';
  const baseRate = baseRateMap[key];

...
import { ArrowLeft, Settings, Brain, Key, Info, Crown, BarChart3, AlertTriangle, Check, UserCog } from "lucide-react";
...
export default function SettingsPage({ onBack, onPricing }: SettingsPageProps) {
  ...
  const { userRoleData, loading: roleLoading, updateUserRole } = useUserRole();
  const [roleForm, setRoleForm] = useState({
    role: userRoleData?.userRole ?? "non-tech",
    level: userRoleData?.technicalLevel ?? "beginner"
  });
  ...
  useEffect(() => {
    if (userRoleData) {
      setRoleForm({ role: userRoleData.userRole, level: userRoleData.technicalLevel });
    }
  }, [userRoleData]);
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6" data-testid="settings-form">
                  <div>
                    ...
                    >
                      <SelectTrigger data-testid="ai-provider-trigger">
                        <SelectValue placeholder="Select AI provider" />
                      </SelectTrigger>
                      <SelectContent data-testid="ai-provider-options">
                  ...
                  {formData.aiProvider !== "platform" && (
                    <div>
                      <Label htmlFor="apiKey" className="flex items-center">
                        <Key className="w-4 h-4 icon-gap" />
                        API Key
                        {requiresUpgrade && (
                      ...
                      {requiresUpgrade ? (
                        <p className="text-sm text-amber-600 mt-1 flex items-center">
                          <AlertTriangle className="w-4 h-4 icon-gap" />
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Switch your role to preview tailored journeys and UI.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <Select
                      value={roleForm.role}
                      onValueChange={(v) => setRoleForm((p) => ({ ...p, role: v as any }))}
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setRoleForm((p) => ({ ...p, level: v as any }))}
                      disabled={roleLoading}
                    >
                      <SelectTrigger data-testid="level-selector">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await updateUserRole(roleForm.role as any, roleForm.level as any);
                        toast({ title: "Role updated", description: "Your role was applied. Journeys will adjust accordingly." });
                      } catch (e) {
                  ...
                  {userRoleData && (
                    <p className="text-xs text-slate-500">
                      Current: {userRoleData.userRole} · {userRoleData.technicalLevel}
                    </p>
                  )}
                </div>
              </CardContent>

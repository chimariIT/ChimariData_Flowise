...
export default function AISettingsPage({ onBack }: AISettingsProps) {
  ...
  const updateSettingsMutation = useMutation<Response, Error, { aiProvider: string; aiApiKey?: string }>({
  ...
  const testApiKeyMutation = useMutation<TestKeyResponse, Error, { provider: string; apiKey: string }>({
    mutationFn: async (data) => {
      const res = await apiRequest("POST", "/api/ai/test-key", data);
      try {
        return (await res.json()) as TestKeyResponse;
      } catch {
        return {} as TestKeyResponse;
      }
    },
    onSuccess: (data) => {
      toast({
  ...
  const providers: Record<string, AIProviderInfo> = providersData?.providers || {};
  const currentProvider: AIProviderInfo | undefined = providers[selectedProvider];
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <Select value={selectedProvider} onValueChange={setSelectedProvider}>
                ...
                <SelectContent>
                  {Object.entries(providers).map(([key, provider]: [string, AIProviderInfo]) => (
